            ...
            <div class="box box-success">
              ...
              <div class="box-body">
                ...
                <div class="col-xs-8">
                  ...
                  <div id="toolbar">
                    <mz:menuTag menuURL="dicts/new">
                      <a href="${pageContext.request.contextPath}/dicts/new" type="button" class="btn bg-maroon btn-flat">添加</a>
                    </mz:menuTag>
                    
                    <mz:menuTag menuURL="dicts/delete">
                      <button type="button" data-ope="delete" class="btn bg-olive btn-flat">删除</button>
                    </mz:menuTag>
                    
                    <mz:menuTag menuURL="dicts/update">
    ...
    $(function (){
    	...
    	$("#toolbar > button").click(function(){
    		...
    		if("delete"==ope){
    			...
    			request.ajax("${pageContext.request.contextPath}/dicts/"+ids,"delete",{},true,function(data){
    				if(data.success){
    					//TODO 确保参数问题？？？？
    			...
    			//界面跳转
    			window.location.href="${pageContext.request.contextPath}/dicts/"+idArray[0]+"/edit";
    		}
    	});
    	
  	    var queryUrl = '${pageContext.request.contextPath}/dicts';
  	    $table = $('#dict_table').bootstrapTable({
        	...
        	//刷新右边的明细
        	$valTable.bootstrapTable('refresh',{query:{dictId:row.id}});
        });
  	     
    	$("#valToolbar > button").click(function(){
    		var idArray = $valTable.bootstrapTable('getAllSelections');
    		if(idArray.length==0){
    		...
    		if("delete"==ope){
    			...
    			request.ajax("${pageContext.request.contextPath}/dictVals/"+ids,"delete",{},true,function(data){
    				if(data.success){
    					//TODO 确保参数问题？？？？
    					$valTable.bootstrapTable('refresh');
    				}else{
    					tip.error(data.msg);
    				}
    			});
  	    ...
  	    $valTable = $('#dictVal_table').bootstrapTable({
  	  	  url: request.contextPath+"/dictVals", //请求后台的URL（*）
            method: 'GET',                      //请求方式（*）
            toolbar: '#valToolbar',                //工具按钮用哪个容器
            striped: true,                      //是否显示行间隔色
            cache: false,                       //是否使用缓存，默认为true，所以一般情况下需要设置一下这个属性（*）
            pagination: true,                   //是否显示分页（*）
            sortable: true,                     //是否启用排序
