...
public class UserCashApplyController {
	 ...
	 * @return
	 */
	@RequestMapping(value={"/{userCashApplyId}/status"}, method={RequestMethod.PUT},headers="X-Requested-With=XMLHttpRequest")
	@AuthAnno
	public @ResponseBody JsonModel updateStatus(@PathVariable("userCashApplyId") Integer userCashApplyId,@RequestParam(required=true) String sign,@RequestParam(required=true) String remark){
		JsonModel jsonModel = new JsonModel();
		if("y".equals(sign)){  //通过
			userCashApplyService.verify(userCashApplyId,StatusConstant.CASH_APPLY_STATUS_SUCCESS,remark);
			jsonModel.setSuccess(true);
			jsonModel.setMsg("审核完成");
		}else if("n".equals(sign)){ //失败
			userCashApplyService.verify(userCashApplyId,StatusConstant.CASH_APPLY_STATUS_FAIL,remark);
			jsonModel.setSuccess(true);
