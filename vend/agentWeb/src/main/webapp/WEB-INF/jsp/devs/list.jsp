                  ...
                  <form id="searchForm" action="" onsubmit="return false;" class="form-inline">
                    <div class="form-group">
                      <select class="form-control" name="key" id="query_key">
                        ...
                        <option id="userOption" value="userName">所属商户</option>
                    ...
                    <%--<div class="col-xs-2">
                      <select class="form-control" name="frothStatus">
                      </select>
                    </div>--%>
...
<script type="text/javascript">
  $(function (){
    ...
    //var frothStatusValues = cache.get(7);

    if(values){
      var $querySelect = $("select[name='state']");
      var result = "<option value=''>请选择在线状态</option>";
      $.each(values,function(key,value){
        result += "<option value="+key+">"+value+"</option>";
      });
      $querySelect.html(result);
    }
    /*if(frothStatusValues){
      var $querySelect = $("select[name='frothStatus']");
      var result = "<option value=''>请选择泡沫状态</option>";
      $.each(frothStatusValues,function(key,value){
        result += "<option value="+key+">"+value+"</option>";
      });
      $querySelect.html(result);
    }*/
    ...
    $table = $('#dev_table').bootstrapTable({
      ...
      columns: [{
        ...
        field: 'frothStatus',
        title: '泡沫状态',
        formatter:frothStatusFormatter
      }*/,{
        field: 'obtain',
        title: '锁开关',
      ...
      onPostBody: function () {
        ...
        $('input[name="Obtain"]').click(function(){
          var devId = $(this).data("id");
          if(type == "agent"){
            alert("您无权开锁");
            return;
          }
          request.ajax("http://www.peshion.com/apiWeb"+"/devs/getObtain/"+devId,"POST",{},true,function(data){
            if(data.success){
        ...
        /*$('[name="Obtain"]').bootstrapSwitch({
            ...
            handleWidth : 50,
              onSwitchChange : function(event, state) {
                    ...
                    request.ajax("${pageContext.request.contextPath}/devs/"+devId+"/status","put",param,false,function(data){
  	    		  	  	  if(data.success){
  	    		  	  	  	  tip.success(data.msg);
  	    		  	  	  }else{
  	    		  	  	  	  tip.error(data.msg);
                    ...
                    request.ajax("http://www.peshion.com/apiWeb/devs/"+devId+"/status","post",param,true,function (data) {
                      if(data.success){
                        tip.success(data.msg);
                      }else{
                        tip.error(data.msg);
        ...
        /* $('input[name="state"]').bootstrapSwitch({
             ...
             handleWidth : 50,
               onSwitchChange : function(event, state) {
                     ...
                     request.ajax("${pageContext.request.contextPath}/devs/"+devId+"/live","put",param,true,function(data){
  	    		  	  	  if(data.success){
  	    		  	  	  	  tip.success(data.msg);
  	    		  	  	  }else{
  	    		  	  	  	  tip.error(data.msg);
    ...
    /*function frothStatusFormatter (value,row,index) {
      return cache.getName(7,value);
    }*/
    ...
    function obtainFormatter(value,row,index) {
      return '<input type="button" data-id="'+row.id+'" name="Obtain" value="开锁" class="btn bg-orange btn-flat" />';
    }
